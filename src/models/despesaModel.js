...
export async function insertDespesa(req) {
  const data = req.data_despesa && req.data_despesa.trim() !== "" ? req.data_despesa : formatarDataLocal(new Date());

  const sql = `
    INSERT INTO despesas (nome_despesa, valor_despesa, data_despesa, fixa)
    VALUES ($1, $2, $3, $4)
    RETURNING *
  `;
  const values = [req.nome_despesa, req.valor_despesa, data, req.fixa];
...
export async function selectDespesa(id_despesa) {
  const sql = `
    ...
    WHERE id_despesa = $1
  `;
  const res = await pool.query(sql, [id_despesa]);
  return res.rows;
}
...
export async function selectDespesas(req) {
  ...
  if (req.data_inicial && req.data_final) {
    ...
    resTotal = await pool.query(sqlTotal, values);
  } else {
    const sql = `
      SELECT id_despesa, nome_despesa, valor_despesa,
        TO_CHAR(data_despesa, 'YYYY-MM-DD') AS data_despesa,
    ...
    resTotal = await pool.query(sqlTotal);
  }
...
export async function updateDespesa(id_despesa, req) {
  const data = req.data_despesa && req.data_despesa.trim() !== "" ? req.data_despesa : undefined;
  ...
  if (data) {
    sql = `
      ...
      RETURNING *
    `;
    values = [req.nome_despesa, req.valor_despesa, data, req.fixa, id_despesa];
  } else {
    sql = `
      UPDATE despesas
      SET nome_despesa = $1,
